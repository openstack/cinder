...
# Copied from Nova's monkey_patch.py
# Re-added Cinder's os.name orig_threading _active patching
...
def _monkey_patch() -> bool:
    ...
    if problems:
        ...
        LOG.warning("Modules with known eventlet monkey patching issues were "
                    ...
                    "importing and not executing cinder code.",
                    ', '.join(problems))

    return True


def patch(backend='eventlet') -> None:
    ...
    if env == '':
        ...
        raise ValueError(
            'Invalid value "%s" for OS_CINDER_DISABLE_EVENTLET_PATCHING '
...
def _poison(*args, **kwargs):
    raise RuntimeError(
        ...
        "OS_CINDER_DISABLE_EVENTLET_PATCHING set to '%s', but then the "
        "service tried to call eventlet.monkey_patch(). This is a bug."
        % os.environ.get('OS_CINDER_DISABLE_EVENTLET_PATCHING', ''))
