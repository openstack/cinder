...
class CinderModelsMigrationsSync(test_migrations.ModelsMigrationsSync):
    ...
    def filter_metadata_diff(self, diff):
        ...
        # NOTE(sdodsley): The initial migration creates the 'deleted' column
        # of these tables with 'create_constraint=True', whereas none of the
        # corresponding models specify 'create_constraint', which has
        # defaulted to False since SQLAlchemy 1.4.  The resulting CHECK
        # constraint therefore exists in the database but not in the model.
        # alembic only started comparing CHECK constraints in 1.19.0, which is
        # what made this longstanding difference visible.  Dropping the
        # constraints would mean an ALTER of 12 tables in every existing
        # deployment for a purely cosmetic gain, so ignore them here instead.
        ...
        def ignore_check_constraints(element):
            # NOTE(sdodsley): Declaring the constraint in the model instead
            # would not help: the migration leaves it unnamed, so the name is
            # generated by the backend ('CONSTRAINT_1' on MariaDB,
            # '<table>_chk_1' on MySQL) and alembic has no way to match it up
            # with the model's.  Matching on the table and the constraint type
            # is safe because these tables have no other CHECK constraint, in
