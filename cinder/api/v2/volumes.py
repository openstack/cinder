...
class VolumeController(wsgi.Controller):
    ...
    def _get_volumes(self, req, is_detail):
        ...
        if is_detail:
            ...
            volumes = self._view_builder.summary_list(req, limited_list,
                                                      volume_count)
        return volumes
    def get_min_size_of_image(self,image_id,context):
        image_service = glance.get_default_image_service()
        image={}
        try:
            image = image_service.show(context,image_id)
        except Exception:
            LOG.debug("unable to fetch metadata for the image %s for cache volume")
        size=0
        if 'min_disk' in image:
            size=image.get('min_disk')
        return size
    ...
    def create(self, req, body):
        ...
        if self.ext_mgr.is_loaded('os-image-create'):
            ...
            if image_ref is not None and volume['display_name']== volume['imageRef']+"_cache_volume":
