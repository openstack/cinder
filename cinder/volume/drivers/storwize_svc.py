...
class StorwizeSVCDriver(san.SanISCSIDriver):
    ...
    def _get_fc_wwpns(self):
        for key in self._storage_nodes:
            ...
            stdout, stderr = self._run_ssh(ssh_cmd)
            self._assert_ssh_return(len(stdout), '_get_fc_wwpns',
                                    ssh_cmd, stdout, stderr)
            wwpns = set(node['WWPN'])
            port_id = []
            port_status = []
            for line in stdout.lstrip().splitlines():
                attr = line.lstrip().split('!')
                if attr[0] == 'port_id':
                    port_id.append(attr[1])
                elif attr[0] == 'port_status':
                    port_status.append(attr[1])
            for i, s in zip(port_id, port_status):
                if 'unconfigured' != s:
                    wwpns.add(i)
