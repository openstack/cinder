...
class StorwizeSSH(object):
    ...
    def lsvolumegroup(self, volumegroup_id_or_name):
        """Return volume group attributes."""
        ssh_cmd = ['svcinfo', 'lsvolumegroup', '-bytes', '-delim', '!',
                   '"%s"' % volumegroup_id_or_name]
        try:
            return self.run_ssh_info(ssh_cmd)[0]
        except exception.VolumeBackendAPIException as ex:
            msg = (_('Unable to fetch the volumegroup: %(vg)s\n'
                     'Exception: %(ex)s') %
                   {'vg': volumegroup_id_or_name,
                    'ex': ex})
            LOG.error(msg)
            raise exception.VolumeBackendAPIException(data=msg)
    ...
    def addsnapshot(self, params):
        ...
        if "list_of_volumes" in params:
            ssh_cmd.extend(['-volumes', params["list_of_volumes"]])
        try:
            return self.run_ssh_check_created(ssh_cmd)
        except Exception:
            with excutils.save_and_reraise_exception():
...
class StorwizeHelpers(object):
    ...
    @staticmethod
    def get_volumegroup_name(group_id):
        # As the storage backend has a character limit for the
        # volumegroup_name, to make it unique we are using the
        # ID generated by cinder manager and taking the first
        # 4 characters and last 5 characters of it to form the
        # unique volumegroup_name that will be created in the
        # storage backend.
        vg = storwize_const.VG_PREFIX
        group_id_str = str(group_id)
        return vg + group_id_str[0:4] + '-' + group_id_str[-5:]

    @staticmethod
    def get_volumegroup_snapshot_name(group_snapshot, grp_snapshot_id=None):
        group_snapshot_id = (
    ...
    def update_volumegroup_snapshot_properties(self, ctxt, snapshots,
                                               group_snapshot=None,
                                               status=None,
        ...
        for snapshot in snapshots:
            ...
            snapshot.metadata['svc_volumegroup_snapshot_name'] = (
                snapshot_metadata['svc_volumegroup_snapshot_name'])
            snapshot.metadata['svc_volumegroup_snapshot_id'] = (
                snapshot_metadata['svc_volumegroup_snapshot_id'])
            snapshot.save()
            snapshots_model.append(
                {'id': snapshot['id'],
                 ...
                 'replication_status': (
                     fields.ReplicationStatus.NOT_CAPABLE)})
        return snapshots_model

    def create_volumegroup_snapshot(self, context, group_snapshot,
                                    snapshots, group_type):
        """Create a volumegroup snapshot based on the group_type"""

        model_update = {'status': fields.GroupSnapshotStatus.AVAILABLE}
        snapshots_model = []
        if group_type == "volume_group_enabled":
            snapshot_metadata = {}
            try:
                params = dict()
                volumegroup_snapshot_name = (
                    self.get_volumegroup_snapshot_name(group_snapshot))
                params["name"] = volumegroup_snapshot_name
                volumegroup_name = self.get_volumegroup_name(
                    group_snapshot.group_id)
                params["volumegroup"] = volumegroup_name
                svc_volumegroup_snapshot_id = self.ssh.addsnapshot(params)
                snapshot_metadata = {
                    'svc_volumegroup_snapshot_name': volumegroup_snapshot_name,
                    'svc_volumegroup_snapshot_id': svc_volumegroup_snapshot_id}
            except exception.VolumeBackendAPIException as err:
                model_update['status'] = fields.GroupSnapshotStatus.ERROR
                msg = (_("Failed to create volumegroup snapshot: "
                         "%(snapshot_name)s"
                         "Error: %(err)s")
                       % {"snapshot_name": volumegroup_snapshot_name,
                          "err": err})
                LOG.error(msg)
            snapshots_model = self.update_volumegroup_snapshot_properties(
                context, snapshots, group_snapshot=group_snapshot,
                status=model_update['status'],
                snapshot_metadata=snapshot_metadata)
        elif group_type == "temporary_volume_group_enabled":
            snapshot_metadata = {}
            try:
                params = dict()
                volumegroup_snapshot_name = (
                ...
                # Get the list of volumes that are present in the group
                # using the snapshot objects.
                list_of_volumes = []
                for snapshot in snapshots:
                    list_of_volumes.append(str(snapshot.volume.name))

                # For group_type as temporary_volume_group_enabled,
                # 'addsnapshot' will be called for a list of volumes.
                params['list_of_volumes'] = ':'.join(list_of_volumes)
                svc_volumegroup_snapshot_id = self.ssh.addsnapshot(params)
                snapshot_metadata = {
                    'svc_volumegroup_snapshot_name': volumegroup_snapshot_name,
                    'svc_volumegroup_snapshot_id': svc_volumegroup_snapshot_id}
    ...
    def delete_volumegroup_snapshot(self, group_snapshot, snapshots,
                                    group_type):
        """Deletes a volumegroup snapshot"""

        model_update = {'status': fields.GroupSnapshotStatus.DELETING}
        snapshots_model = []
        ...
        if group_type == "volume_group_enabled":
            try:
                ...
                # Check if the volume group snapshot exists
                if not self.is_volumegroup_snapshot_exists(params):
                    msg = (_("Tried to delete non-existent volumegroup "
                             "snapshot: %(snapshot_name)s")
                           % {"snapshot_name": volumegroup_snapshot_name})
                    LOG.error(msg)
                    model_update['status'] = (
                        fields.GroupSnapshotStatus.ERROR_DELETING)
                ...
                for snapshot in snapshots:
                    snapshots_model.append(
                        {'id': snapshot['id'],
                         'status': fields.GroupSnapshotStatus.ERROR_DELETING})
                msg = (_("Failed to delete volumegroup snapshot: "
                         "%(snapshot_name)s "
                         "Error: %(err)s")
                       % {"snapshot_name": volumegroup_snapshot_name,
                          "err": err})
            ...
            try:
                params = {}
                params['id'] = (
                    snapshots[0].metadata.get('svc_volumegroup_snapshot_id'))

                if not params['id']:
                    msg = _("Missing svc_volumegroup_snapshot_id "
                ...
                for snapshot in snapshots:
                    snapshots_model.append(
                        {'id': snapshot['id'],
                         'status': fields.GroupSnapshotStatus.ERROR_DELETING})
                msg = (_("Failed to delete the volume group snapshot: "
                         "%(snapshot_name)s with Exception: %(exception)s.") %
                       {'snapshot_name': group_snapshot.name,
                        'exception': err})
                LOG.error(msg)
...
class StorwizeSVCCommonDriver(san.SanDriver,
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Used to indicate whether replication is supported by storage
    ...
    def _update_volumegroup_properties(self, ctxt, volume, group=None):
        volumegroup_name = (
            self._helpers.get_volumegroup_name(group.id) if group else "")
        if not volume.metadata:
            volume.metadata = dict()
        ...
        if (
            ...
            try:
                ...
                if volume_utils.is_group_a_type(
                        ...
                        "temporary_volume_group_enabled"):
                    return model_update
                volumegroup_name = self._helpers.get_volumegroup_name(
                    group.id)
    ...
    def create_group_snapshot(self, context, group_snapshot, snapshots):
        ...
        if (volume_utils.is_group_a_cg_snapshot_type(group_snapshot) or
            ...
            model_update, snapshots_model = (
                self._helpers.create_volumegroup_snapshot(
                    context, group_snapshot, snapshots,
                    "volume_group_enabled"))
        elif volume_utils.is_group_a_type(
                group_snapshot, "temporary_volume_group_enabled"):
            model_update, snapshots_model = (
                self._helpers.create_volumegroup_snapshot(
                    context, group_snapshot, snapshots,
                    "temporary_volume_group_enabled"))
    ...
    def delete_group_snapshot(self, context, group_snapshot, snapshots):
        ...
        if (volume_utils.is_group_a_cg_snapshot_type(group_snapshot) or
            ...
            model_update, snapshots_model = (
                ...
                group_snapshot, "volume_group_enabled"):
            model_update, snapshots_model = (
                self._helpers.delete_volumegroup_snapshot(
